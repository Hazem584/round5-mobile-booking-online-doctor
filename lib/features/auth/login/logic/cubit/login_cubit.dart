...
class LoginWithPhoneCubit extends Cubit<LoginState> {
  ...
  Future<void> loginUser(String phone) async {
    emit(LoginLoading());
    try {
      final result = await phoneLoginRepository.loginWithPhone(phone: phone);

      // هنا بتحول الماب لـ LoginModel
      final loginModel = LoginModel.fromJson(result as Map<String, dynamic>);
